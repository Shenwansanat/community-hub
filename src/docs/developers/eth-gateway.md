...
6. Now, you'll see a "deposit" page where you can input some amount of Kovan ETH to transfer from Layer 1 to Layer 2. You can also click on the `ETH` dropdown to deposit other supported tokens, but we'll stick with ETH for this tutorial. Input a small amount of Kovan ETH and click `DEPOSIT.`
...
9. Now, you just need to wait a minute for your transaction to be processed! When moving assets from Layer 1 to Layer 2, you'll first have to wait for the transaction to be confirmed on Layer 1. It should only take about a minute. You'll also be showered with confetti once your transaction is confirmed. 🎉🎉🎉
...
This tutorial will show you how to perform the whole withdrawal process.
Please ensure you've already completed [the previous tutorial about depositing into L2](#tutorial-depositing-into-l2) before you continue.
...
2. Click on the `Withdraw` button on the main page to switch into withdrawal mode:
...
8. You should be presented with a list of withdrawals that you've made. Now you simply need to wait for the withdrawal challenge period to elapse! On the Optimistic Kovan testnet, this withdrawal period is only 60 seconds. Feel free to grab a drink or go to the bathroom or something while you wait. Click on your withdrawal once the status symbol turns yellow:
...
11. Since you'll be claiming this withdrawal on L1 (Kovan), you'll need to switch back to the Kovan test network in MetaMask before you can continue:
...
15. You can confirm that your withdrawal was completed by going back to [gateway.optimism.io](https://gateway.optimism.io), check L2 balance has decreased and your L1 balance has increased:
...
If you're still confused by something, we'd love to hear how to make this tutorial better.
...
Deposits from Layer 1 into Layer 2 starts when you **send some ETH to the `OVM_L1ETHGateway` contract**.
...
The `OVM_L1ETHGateway` contract proceeds to **send a message** to the `OVM_ETH` contract on Layer 2.
If you want to know more about how this message gets sent, you should check out our page describing the [basics of L1 ⇔ L2 communication](./bridging.html#l1-⇔-l2-communication-basics).

<img src="../../assets/docs/developers/gateway/protocol/2.png" alt="someone did a deposit" width="600"/>

Now we just have to wait a few minutes for the `OVM_ETH` contract on Layer 2 to see this message.

<img src="../../assets/docs/developers/gateway/protocol/3.png" alt="l2 eth contract gets the message" width="600"/>

Once the `OVM_ETH` contract gets the message, it _mints_ some ETH (out of thin air) equal to the amount of ETH originally deposited on Layer 1.

<img src="../../assets/docs/developers/gateway/protocol/4.png" alt="you get some minted ETH" width="600"/>

Badaboom, it's easy as that.
The process of moving ETH back from Layer 2 to Layer 1 is the same thing but in reverse.
...
5. You send a second, final "withdrawal" transaction to the `OVM_L1ETHGateway` on Layer 1 and get your funds back!
6. You are happy.
7. We are happy.
...
Sometimes I struggle with trying to strike a balance between "useful" and "entertaining.” I actually to see this message.
