...
Hello and welcome! If you're looking to find out what it takes to get an app up and running on layer 2, then you've come to the right place. However, if you get stuck or have questions at any time in this process, please reach out on [discord](https://discord.optimism.io) and drop a message in the `#support` channel.
...
Specific EVM opcodes are banned because they can be used to cause non-determinism during a transaction result challenge.
...
Anyway, the point is ** specific opcodes can't be used** (for security reasons), so **we transform them into contract calls** to a particular smart contract (the ["Execution Manager"](https://github.com/ethereum-optimism/optimism/blob/develop/packages/contracts/contracts/optimistic-ethereum/OVM/execution/OVM_ExecutionManager.sol)) that provides the functionality of those opcodes instead.
If you're interested, you can find a complete list of the opcodes that we transform over on the [EVM/OVM comparison guide](../protocol/evm-comparison#replaced-opcodes).

We had a few options for tackling this transformation problem. Still, we stuck with one of the easiest: we built a *slight* [fork of the Solidity compiler](https://github.com/ethereum-optimism/solidity) that automatically handles these transformations when you're compiling your contracts.
...
It means contracts have to be written in Solidity.

::: tip On support for different languages
Unless...

In theory, we can make the same modifications to the compilers for other languages.
We haven't been able to do this yet because we're a relatively small team with a lot on our plates.
If you'd like to help out with this, please reach out to us on [discord](https://discord.optimism.io) and tell us!
Seriously, I'm not just shilling the discord again, and we’d love to help make that a reality, and discord is the best way to get in touch with us.
:::
...
We'll go over the details of using these tools in the following few sections.
...
Less code means a smaller surface for potential issues, and L2 systems are the last place you want to have problems.
We had a few options for approaching this problem but changing a few lines of code in the Solidity compiler was one of the simplest.
We're actively working on ways to toss out the fork of the compiler safely.
...
Under certain circumstances, **constructor arguments can be executable code**.
...
```text
Contract creation code contains unsafe opcodes. Did you use the correct compiler or pass an unsafe constructor argument?
```

<!-- TODO: explain what makes a constructor argument executable -->

This sort of error is most common when using constructor arguments containing many"random" bytes (like contract addresses).
...
This EIP would introduce a new opcode (`BEGINDATA`) to make it possible for us to tell the difference between code and constructor arguments.
...
Because of how we transform specific opcodes into contract calls, **our fork of the Solidity compiler will produce larger bytecode than the standard compiler.**
...
Because of the custom compiler, this development process is the most significant departure from the standard Ethereum development flow.
Depending on your development environment, you're probably going to have to make a few tweaks to compile contracts with our fork of the compiler.
...
I don't know.
We'll figure it out.
The point is: ping us on [discord](https://discord.optimism.io) and let us know how we can help.
:::
...
By running your tests against a local version of Optimistic Ethereum, you can be confident that your contracts will behave correctly on testnet (and eventually mainnet).
Our local Optimistic Ethereum node setup also spins up an Ethereum node on `localhost:9545` to easily [build and test applications that communicate between Layer 1 and Layer 2](https://github.com/ethereum-optimism/optimism/tree/regenesis/0.4.0/examples/l1-l2-deposit-withdrawal).

### Common Gotchas

#### Using the wrong chain ID

If you're getting an error when sending transactions, **please make sure that you're using the correct chain ID**.
...
Alternatively, you can [take a look at how we fund accounts on L2 in our integration tests](https://github.com/ethereum-optimism/optimism/blob/68871b7a9e0989a7b126e800ab1b6b19edab2293/integration-tests/test/shared/utils.ts#L90-L101) for some inspiration for how to do this in your tests.
...
If you reported a bug and we merged a pull request to fix it, but you still see the same issue, make sure to update and rebuild your local Optimistic Ethereum setup before opening up another issue:
...
#### Running out of gas, but the receipt says you didn't use the full gas limit
...
If the block gas limit is 9,000,000 and you try to send a transaction with 9,000,000 gas, you'll only get to use about ~8,000,000 gas.
...
As much as we'd love to hit the big red YOLO button, we do need to be very careful.
We appreciate your patience while working hard to get everything in place for a smooth public mainnet launch 💗.
:::
...
Although we're currently limiting contract deployment on the mainnet, you might still want to connect to the network.
...
Except we offer more, Optimistic.
If you're looking for something quick and easy to tackle next, you might want to look at the [Official™ Optimism Tutorial](https://github.com/ethereum-optimism/optimism-tutorial).
